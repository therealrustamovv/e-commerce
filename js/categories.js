...
const select = document.querySelector(".categories__select");
...
function getFavouriteProducts() {
  let results = categoryProducts.filter((pr) => pr.name);
  let pages = Math.ceil(results.length / CTGLIMIT);
  headerTitle.textContent = localStorage.getItem(CATEGORY);
  headerYou.textContent = localStorage.getItem(CATEGORY);
  ...
  if (pages > 1) {
    pagination.innerHTML = `
        <li class="page-item">
          <button class="page-link ${activePage === 1 ? "active" : ""}"  ${
    ...
    pagination.innerHTML += `
        <li class="page-item">
          <button class="page-link ${activePage === pages ? "active" : ""}"  ${
  ...
  let start = (activePage - 1) * CTGLIMIT;
  let end = activePage * CTGLIMIT;
